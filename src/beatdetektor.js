...
const blink = throttle((canvasCtx, canvas) => {
...
BeatDetektor.prototype.reset = function () {
    ...
    for (var i = 0; i < this.config.BD_DETECTION_RANGES; i++) {
        ...
        this.ma_bpm_range[i] = this.maa_bpm_range[i] =
            60.0 / this.BPM_MIN +
            (60.0 / this.BPM_MAX - 60.0 / this.BPM_MIN) * (i / this.config.BD_DETECTION_RANGES);
...
BeatDetektor.prototype.process = function (timer_seconds, fft_data, canvasCtx, canvas) {
    if (!this.last_timer) {
    ...
    for (x = 0; x < fft_data.length; x += range_step) {
                ...
                for (i = 0; i < this.config.BD_REWARD_TOLERANCES.length; i++) {
                    if (
                        ...
                        this.detection_quality[range] +=
                            this.config.BD_QUALITY_REWARD * this.config.BD_REWARD_MULTIPLIERS[i];
                        rewarded = true;
                    }
                }
            ...
            if (rewarded) {
                ...
                if (
                    ...
                    this.ma_bpm_range[range] -=
                        (this.ma_bpm_range[range] - trigger_gap) * this.config.BD_QUALITY_STEP;
                    this.maa_bpm_range[range] -=
        ...
        if (
            ...
            (det && Math.abs(this.ma_bpm_range[range] - this.current_bpm) > this.bpm_offset)
        )
            this.detection_quality[range] -=
                this.detection_quality[range] *
                this.config.BD_QUALITY_STEP *
        ...
        for (x = 0; x < this.config.BD_DETECTION_RANGES; x++) {
            ...
            if (
                ...
                if (this.ma_bpm_range[x] < bpm_ceil && this.ma_bpm_range[x] > bpm_floor) {
                    ...
                    draft_float =
                        Math.abs(Math.ceil(draft_float) - (60.0 / this.current_bpm) * 1000.0) <
                        Math.abs(Math.floor(draft_float) - (60.0 / this.current_bpm) * 1000.0)
    ...
    // console.log('current_bpm', this.current_bpm);
    ...
    if (contest_max > this.config.BD_FINISH_LINE) {
        for (var contest_i in this.bpm_contest_lo) {
            this.bpm_contest_lo[contest_i] =
                (this.bpm_contest_lo[contest_i] / contest_max) * this.config.BD_FINISH_LINE;
        }
    }
    ...
    for (contest_i in this.bpm_contest) {
        this.bpm_contest[contest_i] -=
            this.bpm_contest[contest_i] * (this.last_update / this.config.BD_DETECTION_RATE);
    }

    // decay contest values from last loop
    for (contest_i in this.bpm_contest_lo) {
        this.bpm_contest_lo[contest_i] -=
            this.bpm_contest_lo[contest_i] * (this.last_update / this.config.BD_DETECTION_RATE);
    }
    ...
    for (var i = 0; i < det.config.BD_DETECTION_RANGES; i++) {
        ...
        if (det_val > 1.0) {
            ...
            if (det_val > this.vu_levels[i]) this.vu_levels[i] = det_val;
                ...
                this.vu_levels[i] -=
                    (this.vu_levels[i] - det_val) * det.last_update * (1.0 / det.current_bpm) * 3.0;
        } else {
